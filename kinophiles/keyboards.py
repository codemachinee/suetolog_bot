...
def get_edit_list_category_keyboard() -> InlineKeyboardMarkup:
    ...
    buttons = [
        [
            InlineKeyboardButton(
                text="🎬 Фильмы",
                callback_data=KinophilesCallback(action="edit_items_list", category="фильм", page=1).pack()
            ),
            InlineKeyboardButton(
                text="📺 Сериалы",
                callback_data=KinophilesCallback(action="edit_items_list", category="сериал", page=1).pack()
            )
        ],
        [
            InlineKeyboardButton(
...
def get_other_users_keyboard(lists: List[Tuple[int, str, int]]) -> InlineKeyboardMarkup:
    ...
    for list_id, list_name, user_id in lists:
        buttons.append([
            InlineKeyboardButton(
                text=list_name,
                callback_data=KinophilesCallback(action="view_list", list_id=list_id).pack()
            )
        ])
    buttons.append([
        InlineKeyboardButton(
...
def get_view_category_keyboard(list_id: int) -> InlineKeyboardMarkup:
    ...
    buttons = [
        [
            InlineKeyboardButton(
                text="🎬 Фильмы",
                callback_data=KinophilesCallback(action="view_category", list_id=list_id, category="фильм", page=1).pack()
            ),
            InlineKeyboardButton(
                text="📺 Сериалы",
                callback_data=KinophilesCallback(action="view_category", list_id=list_id, category="сериал", page=1).pack()
            )
        ],
        [
            InlineKeyboardButton(
...
def get_confirmation_keyboard(action: str, category: str) -> InlineKeyboardMarkup:
    ...
    buttons = [
        [
            InlineKeyboardButton(
                text="Да, удалить все",
                callback_data=KinophilesCallback(action=action, category=category).pack()
            ),
            InlineKeyboardButton(
                text="Нет, вернуться",
                callback_data=KinophilesCallback(action="edit_items_list", category=category, page=1).pack()
            )
        ]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def get_view_items_keyboard(list_id: int, category: str, page: int, total_pages: int) -> InlineKeyboardMarkup:
    ...
    if page > 1:
        pagination_buttons.append(
            InlineKeyboardButton(
                text="⬅️ Назад",
                callback_data=KinophilesCallback(action="view_category", list_id=list_id, category=category, page=page - 1).pack()
            )
        )
    if page < total_pages:
        pagination_buttons.append(
...
def get_edit_items_keyboard(
    ...
    for item in items:
        ...
        if not is_selected:
            item_buttons.append(
                InlineKeyboardButton(
                    text="✏️", # Edit button
                    callback_data=KinophilesCallback(action="edit_item_start", item_id=item['id'], category=category, page=page).pack()
                )
            )
    ...
    if page > 1:
        pagination_buttons.append(
            InlineKeyboardButton(
                text="⬅️ Назад",
                callback_data=KinophilesCallback(action="edit_items_list", category=category, page=page - 1).pack()
            )
        )
    if page < total_pages:
        pagination_buttons.append(
    ...
    buttons.append([
        InlineKeyboardButton(
            ...
            callback_data=KinophilesCallback(action="add_item", category=category).pack()
        )
    ])
    buttons.append([
        InlineKeyboardButton(
            text="💥 Удалить все",
            callback_data=KinophilesCallback(action="delete_all_confirm", category=category).pack()
        )
    ])
    buttons.append(
        [
...
def get_edit_field_keyboard(item_id: int, category: str, page: int) -> InlineKeyboardMarkup:
    """Возвращает клавиатуру для выбора поля для редактирования."""
    buttons = [
        [
            InlineKeyboardButton(
                text="Название",
                callback_data=KinophilesCallback(action="choose_field", item_id=item_id, field="title").pack()
            )
        ],
        [
            InlineKeyboardButton(
                text="Ссылка",
                callback_data=KinophilesCallback(action="choose_field", item_id=item_id, field="link").pack()
            )
        ],
        [
            InlineKeyboardButton(
                text="Примечание",
                callback_data=KinophilesCallback(action="choose_field", item_id=item_id, field="note").pack()
            )
        ],
        [
            InlineKeyboardButton(
                text="⬅️ Отмена",
                callback_data=KinophilesCallback(action="edit_items_list", category=category, page=page).pack()
            )
        ]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
