...
class Davinci:
    ...
    async def answer(self):
        saved_messages_davinci.insert(
            len(saved_messages_davinci) + 1, {"role": "user", "text": f"{self.text}"}
        )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                try:
                    answer = (await response.json())["result"]["alternatives"][0][
                        "message"
                    ]["text"]
                    # await self.bot.send_message(self.message.chat.id, f'{answer}')
                    await self.bot.edit_message_text(
                        f"{answer}", message.chat.id, message.message_id
                    )
                    saved_messages_davinci.insert(
                        len(saved_messages_davinci) + 1,
                        {"role": "assistant", "text": f"{str(answer)}"},
                    )
                    ...
                    logger.exception("Ошибка в yandex_services/Davinci/answer", e)
        ...
        async with session.post(
            ...
            try:
                ...
                await bot.send_message(
                    paswords.group_id,
                    "Короче с др брат, ты и так все знаешь.."
                    "а эта суета с лишними словами для слабых духом"
                    "мы же с тобой сильные... обнял",
                )
                logger.exception("Ошибка в yandex_services/Artur_pozdravlyaet", e)
...
class YaDisk:
    ...
    async def save_photo(self):
        try:
            ...
            await self.bot.send_message(
                self.message.chat.id, f"Отправка не удалась. Сервер перегружен, {e}"
            )
    ...
    async def save_doc(self):
        try:
            ...
            try:
                ...
                await self.bot.send_message(
                    self.message.chat.id, "документ успешно загружен"
                )
            except yadisk.exceptions.ParentNotFoundError:
                y.mkdir(src)
                y.upload(
            ...
            await self.bot.send_message(
                self.message.chat.id, "ОШИБКА! Документ слишком большой"
            )
        except Exception as e:
    ...
    async def save_video(self):
        try:
            ...
            try:
                ...
                await self.bot.send_message(
                    self.message.chat.id, "видео успешно загружено"
                )
            except yadisk.exceptions.ParentNotFoundError:
                y.mkdir(src)
                y.upload(
